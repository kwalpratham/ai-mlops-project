...
terraform {
  ...
  #   key    = "text-classifier/terraform.tfstate"
  #   region = "us-east-1"
  # }
}
...
# ── VPC & Networking (use default for simplicity) ────────────────────────────
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "MODEL_TYPE", value = var.model_type },
      { name = "MODEL_PATH", value = "/app/models/${var.model_type}" },
